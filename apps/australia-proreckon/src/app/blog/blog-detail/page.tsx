...
export default function CaseStudyDetail({ params: { slug } }: { params: { slug: string } }) {
    ...
    const handleNextBlogDetail = () => {
        if (foundPost) {
            ...
            if (index === blogData.length - 1) {
                nextId = blogData?.[0]?.id as number;
            } else {
                nextId = blogData?.[index + 1]?.id as number;
            }
    ...
    const handlePrevBlogDetail = () => {
        if (foundPost) {
            ...
            if (index === blogData?.[0]?.id as number - 1) {
        ...
        <>
            <div className="overflow-x-hidden">
                ...
                <main className="content">
                    {/** Change This */}
                    <BreadcrumbItem link={foundPost.title} img="/images/banner/about4.jpg" title={foundPost.title} desc="" />

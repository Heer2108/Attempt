        ...
        <div className="flex max-lg:flex-col-reverse gap-y-10">
          <div className="w-full lg:w-1/3 lg:pr-[55px]">
            ...
            <div className="recent-post-block md:mt-10 mt-6">
              <div className="recent-post-heading heading7">Other Posts</div>
              <div className="list-recent-post flex flex-col gap-6 mt-4">
                {blogData?.filter(item => item?.id !== data?.id).slice(0,5)?.map((item) => (
                  <Link
                    key={item.id}
          ...
          <div className="w-full lg:w-2/3">
            <div className="blog-paragraph">
              <div className="paragraph-heading">
                <span className="caption2 py-2 px-4 bg-surface rounded-2xl hover:bg-black hover:text-white capitalize">
                  {data?.tag || "Design"}
                </span>
                <div className="heading4 mt-4">
                  {data?.title ||
                    "Earn good money and make you very successful creative Business"}
                </div>
                ...
                <div className="bg-img mt-8">
                  <Image
                    ...
                    src={data?.img || "/images/blog/930x593.jpg"}
                    alt="img"
                    loading='lazy'
                  />
                </div>
              ...
              <div className="paragraph-content mt-8">
                <div className="body2 text-secondary">{data?.ingress}</div>
                {data?.quote && (
                  <div className="review pl-4 py-2 border-l-2 border-line mt-4">
                    <div className="body2 italic">{data?.quote?.text}</div>
                    <span className="text-xs uppercase font-bold mt-4">
                      {data?.quote?.author}
                    </span>
                  </div>
                )}
                ...
                <div className="paragraph-content mt-8">
                  ...
                  <div className="list-feature mt-8">
                    <div className="grid lg:grid-cols-1 gap-y-3">
                      {item?.listTitle ? item?.listTitle : ""}
                      <div className="w-full gap-y-3 flex flex-col">
                        {item?.listContent?.length
                          ? item?.listContent?.map((item) => (
                              <div className="item flex items-center gap-3">
                                <Icon.CheckCircle
                                  weight="fill"
                                  className="text-xl min-w-[1em] text-blue"
                                />
                  ...
                  {item?.break && <div className="mt-5 border border-line"></div>}
              ...
              <div className="infor-below flex max-sm:flex-wrap items-center justify-between gap-6 py-5">
                <div className="prev-block">
                  <div
                    ...
                    onClick={handlePrevBlogDetail}
                  >
                    <div className="text-button-uppercase text-blue">
                      Previous
                    </div>
                    <div className="heading7 mt-1">
                      {data?.id === 1
                        ? blogData[blogData.length - 1]?.title
                        : blogData[data.id - 2].title}
                    </div>
                  </div>
                </div>
